...
from twonet_ac_agent import Agent, Policy, Value
...
def train(env_name, print_things=True, train_run_id=0, train_episodes=5000):
    ...
    for episode_number in tqdm(range(train_episodes)):
        ...
        while not done:
            ...
            timesteps += 1
            # Task 4
            # if timesteps % 10 == 0 and timesteps != 0:
            #     agent.episode_finished(episode_number)

        if print_things and episode_number%500==0:
    ...
    if print_things:
        ...
        plt.title("Reward history "+task_name)
        plt.savefig(task_name+".pdf")
        plt.show()
    ...
    torch.save(agent.policy.state_dict(), "model_%s_%d_%s.mdl" % (env_name, train_run_id, task_name))
    return data
